...
function UploadPageInner() {
  ...
  // Restore parsed events after any page load (OAuth redirect or manual reload)
  useEffect(() => {
  ...
  const handleParse = async () => {
    ...
    try {
      // Extract text client-side (avoids server-side PDF parsing issues)
  ...
  const handleSync = async () => {
    ...
    try {
      ...
      // Optionally sync to Google Calendar if connected
      if (googleConnected) {
    ...
    <div className="mx-auto max-w-3xl space-y-6">
      <div>
        ...
        <p className="mt-1 text-[13px] text-muted-foreground">
          Drop your syllabus PDFs to extract deadlines and sync them to Google Calendar.
        </p>
      </div>
